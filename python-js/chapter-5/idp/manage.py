...
This file is a custom Django management utility script for administrative tasks,
specifically tailored to run the development server over HTTPS using django-sslserver.
It handles SSL certificate and key management, including decryption of an encrypted
private key and creation of a certificate chain file by combining server, intermediate,
and root certificates. The script sets up environment variables for Django settings and
superuser credentials, then launches the server on port 8443 with the specified SSL
